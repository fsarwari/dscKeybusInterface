...
#ifndef ntpTimeZone
#define ntpTimeZone TZ_America_New_York  // Set the time zone (includes DST): https://github.com/esp8266/Arduino/blob/master/cores/esp8266/TZ.h
#endif
#ifndef ntpServer
const char* ntpServer = "pool.ntp.org";  // Set the NTP server (default)
#endif
#ifndef timePartition
#define timePartition 1  // Set the partition to use for setting the time
#endif
...
unsigned long ntpSyncAttemptTime = 0;  // Track when we last attempted sync
const unsigned long NTP_SYNC_RETRY_INTERVAL = 30000;  // Retry every 30 seconds if partition not ready
...
void setup() {
  ...
  time(&ntpNow);
  while (ntpNow < 1606784461) {  // Wait for valid NTP time (after 2020-12-01)
    Serial.print(".");
    delay(2000);
    time(&ntpNow);
  }
  Serial.print(F("synchronized: "));
  Serial.println(ctime(&ntpNow));
  ...
  ArduinoOTA.setHostname("dsc-keybus-esp8266");
  // Note: Empty password means no authentication required
  ArduinoOTA.setPassword("");  
  ArduinoOTA.setPort(8266);  // Explicitly set OTA port (default is 8266)
  ArduinoOTA.setRebootOnSuccess(true);  // Auto-reboot after successful update
  ...
  ArduinoOTA.onStart([]() {
    ...
    Serial.println(type);
    // Disconnect MQTT to free up resources during OTA
    mqtt.disconnect();
  });
  ...
  ArduinoOTA.onError([](ota_error_t error) {
    ...
    if (error == OTA_AUTH_ERROR) {
      ...
      Serial.println("Begin Failed - Not enough space or invalid partition");
    } else if (error == OTA_CONNECT_ERROR) {
      Serial.println("Connect Failed");
    } else if (error == OTA_RECEIVE_ERROR) {
      Serial.println("Receive Failed - Network issue");
    } else if (error == OTA_END_ERROR) {
      Serial.println("End Failed - Flash write/verify failed. May need full erase via serial.");
      Serial.println("Try: pio run -t erase && pio run -t upload (via serial)");
    }
  });
  ...
  // Start MDNS responder for hostname resolution
  if (MDNS.begin("dsc-keybus-esp8266")) {
    Serial.println(F("MDNS responder started"));
  }
  
  Serial.print(F("OTA Updates enabled on "));
...
void loop() {
  // Handle OTA first and frequently - it needs to respond quickly to incoming connections
  ArduinoOTA.handle();
  yield();  // Give WiFi stack time to process
  
  // Only process other tasks if OTA is not active
  if (!ArduinoOTA.getCommand()) {
    ...
    if (!ntpSynced) {
      ...
      // Try to sync at the offset second, or retry periodically if partition wasn't ready
      bool shouldSync = false;
      if (ntpSecond == ntpOffset) {
        shouldSync = true;
      } else if (ntpOffset > 55 && ntpSecond == 0) {
        shouldSync = true;
      } else if (ntpSyncAttemptTime > 0 && (currentTime - ntpSyncAttemptTime) >= NTP_SYNC_RETRY_INTERVAL) {
        // Retry if we've waited long enough and partition might be ready now
        shouldSync = true;
      }
      ...
      if (shouldSync) {
        ...
        //   Serial.print(".");
        //   if (ntpMonth < 10) Serial.print("0");
        //   Serial.print(ntpMonth);
        //   Serial.print(".");
        //   if (ntpDay < 10) Serial.print("0");
        //   Serial.print(ntpDay);
        //   Serial.print(" ");
        //   if (ntpHour < 10) Serial.print("0");
  ...
  if (dsc.statusChanged) {      // Checks if the security system status has changed
    ...
    // Checks for a panel timestamp and synchronizes with NTP if needed
    if (dsc.timestampChanged) {
      ...
      bool timeDiffers = ( dsc.hour != ntpHour || dsc.minute != ntpMinute);
      // print dsc time before change:
      Serial.print(F("\nDSC Time: "));
      if (dsc.hour < 10) Serial.print("0");
      Serial.print(dsc.hour);
      Serial.print(":");
      if (dsc.minute < 10) Serial.print("0");
      ...
      if (timeDiffers) {
        Serial.println(F("Time differs\n"));

        ntpOffset = ntpSecond;
        ntpSynced = false;
        // only update time if last updated time is more than 60 seconds ago
        if (ntpNow - lastUpdatedTime > 60 || lastUpdatedTime == 0) {
          Serial.println(F("Update time: "));

          // time reset string format: *6{mastercode}1hhmmMMDDYY#
          String timeEntry = String("*6") + String(accessCode) + String("1") + String(ntpHour) + String(ntpMinute) + String(ntpMonth) + String(ntpDay) + String(ntpYear) + String("#");
          Serial.printf("Time entry: %s\n", timeEntry.c_str());
          // dsc.write(timeEntry.c_str());
          // write one char at a time:
          for (int i = 0; i < timeEntry.length(); i++) {
            dsc.write(timeEntry[i]);
            delay(100);
          }
          // record last updated time
          lastUpdatedTime = ntpNow;
          Serial.printf("Last updated time: %s\n", ctime(&lastUpdatedTime));
        }

      } else {
        Serial.println(F("Time is synchronized."));
        ntpSynced = true;
      }
    }
